...
public class Palavra extends ObjetoDominioImpl {
	...
	public static Palavra criar(long id, String palavra, Tema tema) {
		if (letraFactory == null) {
			throw new RuntimeException("O LetraFactory precisa ser iniciado!");
		}
	...
	public Letra[] getLetras() {
		if (this.palavra == null) {
			throw new RuntimeException("A palavra deve ser inicializada!");
		}
	...
	public boolean comparar(String palavra) {
		...
		for (int posicao = 0; posicao < getTamanho(); posicao++) {
			char letraPalavra = Character.toUpperCase(this.palavra[posicao].getCodigo());
			char letraParam = Character.toUpperCase(palavra.charAt(posicao));
			if (letraPalavra != letraParam) {
				return false;
			}
		}
